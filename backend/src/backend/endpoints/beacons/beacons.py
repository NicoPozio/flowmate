...
class BeaconCreateRequest(BaseModel):
    ...
    inactivity_timeout_minutes: Optional[int] = None # NUOVO PARAMETRO
...
def register_beacon(req: BeaconCreateRequest, conn: mariadb.Connection = Depends(db_connection)):
    ...
    query = """
        ...
        zone_icon, weekday_from_time, weekday_to_time, weekend_from_time, weekend_to_time, associated_hobby_id, inactivity_timeout_minutes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    try:
        params = (
            ...
            req.associated_hobby_id, req.inactivity_timeout_minutes
        )
...
def update_beacon(beacon_id: str, req: BeaconCreateRequest, conn: mariadb.Connection = Depends(db_connection)):
    query = """
        ...
        SET zone_name = ?, allow_notifications = ?, zone_icon = ?, 
            weekday_from_time = ?, weekday_to_time = ?, weekend_from_time = ?, weekend_to_time = ?, inactivity_timeout_minutes = ?
        WHERE beacon_id = ?
    """
    execute_query(conn, query, (req.zone_name, req.allow_notifications, req.zone_icon, 
                                req.weekday_from_time, req.weekday_to_time, 
                                req.weekend_from_time, req.weekend_to_time, req.inactivity_timeout_minutes, beacon_id), fetch=False)
